...
class TextStyleTransferOutputSelector: 
    ...
    def select_outputs_batch(
        ...
        output_contents = []
        output_styles = []
        for lmbda, src, hypos, label in tqdm(zip(lmbdas,source_texts, generated_texts, 
                                          target_labels),
                                      total=len(source_texts)):
            hypos = [h for h in hypos if len(h) > 0]
            w_scores, contents, styles = self.compute_sample_rewards(lmbda, src, hypos, 
                                                                    label)
            max_w_score = torch.tensor(w_scores).float().max()
            top_index = w_scores.index(max_w_score)

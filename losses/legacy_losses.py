"""Legacy losses kept only for checkpoint/back compatibility with pre-v2 runs.

These are the original exploratory implementations (inconsistent reward
scaling, no std epsilon guards). New work should use the unified family in
loss_functions.py: `grpo` (grpo_v2_loss) and `make_rrebel_loss(...)`.

Provenance: bodies match the 2026-07 v2 working tree (the code the v2 runs
actually executed), which had drifted from the last pre-refactor commit —
e.g. drgo_loss's 0.1 scale and grpo_loss's hardcoded beta=0. Do not "fix"
these; they exist to reproduce old runs exactly.
"""
import math
...
from .loss_functions import (score_scaler_fnc, get_log_prob,
                             get_log_prob_diff, get_per_token_log_prob,
                             get_pairwise_diff)
...
def drgo_loss(
    ...
    scores_tensor = 0.1*score_scaler_fnc(lmbda_)*scores_tensor.contiguous().view(num_src, -1)
    _, indices = torch.sort(scores_tensor, dim=-1)

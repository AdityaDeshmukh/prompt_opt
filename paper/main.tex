...
seed above \emph{every} GRPO seed. The GRPO baseline was first debugged (a
reference-anchor defect worth $+4.1$ points) and then given the same
exploration lever \rrebel{} has (a further $+1.8$), and the gap persisted. We
also report, rather than suppress, two limitations that bound how far this
comparison can be pushed: the reported metric is a $10$-sentence
development-set score rather than the held-out test split, and because the
pinned KL reference is numerically the uniform distribution, the GRPO arms
carry an entropy bonus rather than a genuine trust region --- so the
plain-GRPO arm collapses by step $500$ and is a collapse diagnostic, not a
converged baseline.
\end{abstract}
